...
def get_updated_configs(config: InferenceConfig):
    """
    Generate a list of configurations for each hidden layer in a Qwen2 model.
...
def _helper_concat_and_delete_qkv(Qwen2_state_dict, layer_num, attr):
    """
    Helper function to concatenate and delete QKV attributes for fusedqkv (weight or scale).
    Args:
        Qwen2_state_dict: The state dictionary containing model weights
        layer_num: The index of the layer to process
        attr: The attribute to process ('weight' or 'scale')
    """
    Qwen2_state_dict[f"layers.{layer_num}.self_attn.Wqkv.{attr}"] = torch.cat(
        [
            Qwen2_state_dict[f"layers.{layer_num}.self_attn.q_proj.{attr}"],
            Qwen2_state_dict[f"layers.{layer_num}.self_attn.k_proj.{attr}"],
            Qwen2_state_dict[f"layers.{layer_num}.self_attn.v_proj.{attr}"],
        ],
    )
    del Qwen2_state_dict[f"layers.{layer_num}.self_attn.q_proj.{attr}"]
    del Qwen2_state_dict[f"layers.{layer_num}.self_attn.k_proj.{attr}"]
    del Qwen2_state_dict[f"layers.{layer_num}.self_attn.v_proj.{attr}"]
...
def convert_state_dict_to_fused_qkv(Qwen2_state_dict, cfg: InferenceConfig):
    ...
    for l in range(cfg.num_hidden_layers):  # noqa: E741
        _helper_concat_and_delete_qkv(Qwen2_state_dict, l, "weight")
        if (
            ...
            _helper_concat_and_delete_qkv(Qwen2_state_dict, l, "scale")
...
class NeuronQwen2MLP(nn.Module):
    ...
    def __init__(self, config: InferenceConfig):
        ...
        if self.neuron_config.quantized_mlp_kernel_enabled and self.quantize_clamp_bound == float(
            ...
            logging.warning(
                "quantize_clamp_bound is not specified in NeuronConfig. We will use the default value of 1200 for Qwen2 models in quantized kernels."
            )
